...
Stack: Supabase · PostgreSQL · RLS · RPC · PostgREST Para tarefas operacionais, a IA deve usar primeiro o Índice Operacional abaixo e ler apenas a seção relevante.
...
Criação ou alteração de tabela, naming, colunas obrigatórias, modelagem → leia a seção 1. Schema e Design de Tabelas

Chaves estrangeiras, ON DELETE RESTRICT, ON DELETE CASCADE, soft delete → leia a seção 1.3 Chaves Estrangeiras e Regras de Exclusão

RLS, policies, controle de acesso, acesso por admin, acesso por organização → leia a seção 2. Segurança — Row Level Security (RLS)

Trigger de updated_at, automações, criação de perfil, sincronização com auth.users → leia a seção 3. Automação com Triggers e Funções

RPC, função SQL, lógica transacional, SECURITY INVOKER, SECURITY DEFINER → leia a seção 4. Lógica de Negócio com RPC

Exposição de dados ao frontend, views, contrato estável entre back e front → leia a seção 5. Exposição de Dados para o Front-end

Validação de nova tabela → leia a seção 6. Checklist de Nova Tabela

Criação de migration nova, convenções de nome, fluxo com Supabase CLI → leia a seção 7. Migrações

Regras específicas do Dyad, SECURITY_RULES.md, proteção de migrations → leia a seção 8. Notas Específicas para o Dyad

Ordem recomendada de leitura
Toda tarefa de backend:

AI_RULES.md
src/contexts/<name>/README.md do contexto afetado
Seção relevante deste backend.md
Tarefa full-stack:

AI_RULES.md
src/contexts/<name>/README.md
Seção relevante deste backend.md
Seção relevante de docs/frontend.md
Regras de navegação
...
Segurança por padrão: Acesso negado sempre. Toda permissão é concedida explicitamente via RLS.
Integridade no banco: Validação, consistência e regras de negócio vivem no PostgreSQL — não no front-end.
Fonte única de verdade: Operações atômicas e lógica complexa ficam em funções RPC.
Convenções consistentes: Qualquer desenvolvedor deve entender o schema ao olhar pela primeira vez.
Acesso controlado por papel: Nenhum usuário acessa o sistema sem aprovação de um admin.
...
O domínio é validado a cada carga de dados (não apenas no login)
Sessões restauradas (que não passam pelo AuthCallback) também têm o domínio verificado
Revogar um domínio em allowed_domains remove o acesso de todos os usuários daquele domínio imediatamente
